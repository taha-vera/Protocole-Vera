(() => {
  ...
  function prepare_sjcl_random_generator() {
    ...
    while (!sjcl_default.random.isReady(void 0)) {
  ...
  var PartiallyBlindRSA = class _PartiallyBlindRSA {
    params;
    static NAME = "RSA-PSS";
    constructor(params) {
      this.params = params;
    ...
    // in bits and in bytes of the modulus, and the hash function used.
    async extractKeyParams(key, type) {
      if (key.type !== type || key.algorithm.name !== _PartiallyBlindRSA.NAME) {
        throw new Error(`key is not ${_PartiallyBlindRSA.NAME}`);
      }
      if (!key.extractable) {
        throw new Error("key is not extractable");
      }
    ...
    async blindSign(privateKey, blindMsg, info) {
      ...
      let s;
      if (this.params.supportsRSARAW) {
        const { privateKey: privateKey2 } = await _PartiallyBlindRSA.bigKeyPairToCryptoKeyPair({ secretKey: sk_derived, publicKey: pk_derived }, {
    ...
    async finalize(publicKey, msg, info, blindSig, inv) {
      ...
      }, { name: _PartiallyBlindRSA.NAME, hash: this.params.hash }, false, ["verify"]);
      const algorithm = { name: _PartiallyBlindRSA.NAME, saltLength: this.params.saltLength };
    ...
    static async generateKey(algorithm, generateSafePrimeSync = generateSafePrime) {
      ...
      const sk = { n, p, q, d };
      const pk = { e, n };
      return _PartiallyBlindRSA.bigKeyPairToCryptoKeyPair({
        secretKey: sk,
        publicKey: pk
      }, algorithm, true);
    }
    generateKey(algorithm, generateSafePrimeSync = generateSafePrime) {
      return _PartiallyBlindRSA.generateKey({ ...algorithm, hash: this.params.hash }, generateSafePrimeSync);
    }
    async verify(publicKey, signature, message, info) {
      ...
      return crypto.subtle.verify({ name: _PartiallyBlindRSA.NAME, saltLength: this.params.saltLength }, pk_derived_key, signature, msg_prime);
    }
    ...
    static async bigKeyPairToCryptoKeyPair({ secretKey, publicKey }, algorithm, extractable) {
      ...
      }, { ...algorithm, name: _PartiallyBlindRSA.NAME }, extractable, ["sign"]);
      const pk = await crypto.subtle.importKey("jwk", {
        ...
        n: sjcl_default.codec.base64url.fromBits(n.toBits(0)),
        e: sjcl_default.codec.base64url.fromBits(e.toBits(0))
      }, { ...algorithm, name: _PartiallyBlindRSA.NAME }, extractable, ["verify"]);
